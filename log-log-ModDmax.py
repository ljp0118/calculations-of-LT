...
for i in range(183):
    ...
    if dmax_speed==x[len(x)-1] or dmax_speed==x[0]:
        ...
        dmax_speed=0
        data = {
            'Log_log_ModDmax_LT_speed': dmax_speed,
            'Log_log_ModDmax_LT': dmax_la
        }
        new_row = pd.DataFrame([data])
        df_la = pd.concat([df_la, new_row], ignore_index=True)
        print(f"第{i}个图像的log-log-ModDmax乳酸阈的运动强度是：{round(dmax_speed, 8)} km/h, 对应的血乳酸浓度是：{round(dmax_la, 8)} mmol/L")
        continue
    else:
        data = {
            'Log_log_ModDmax_LT_speed': dmax_speed,
        ...
        plt.xlabel('Speed')
        plt.ylabel('Lactate')
        plt.title('log-log-ModDmax')
        plt.legend()
        plt.savefig(f"../LT/imgs/log-log-ModDmax/log-log-ModDmax_{i}.png")
        plt.show()
        print(f"第{i}个图像的log-log-ModDmax乳酸阈的运动强度是：{round(dmax_speed, 8)} km/h, 对应的血乳酸浓度是：{round(dmax_la, 8)} mmol/L")

df_la.to_csv('../LT/csv/Log_log_ModDmax.csv', index=True)
